  ...
  <div style="display: flex">
    ...
    >
      <template v-for="item in selectedMenus">
        ...
        >
          <div
            :class="['menu', { hovered: data.id ? item.id === data.id : false }]"
            @click="() => mouseEnter(item)"
          >
            <AsyncIcon
              :icon="item.icon"
              style="height: 45px; display: flex; justify-content: center; align-items: center"
              size="20px"
            />
    ...
    <a-drawer
      ...
      :open="open"
      :mask="false"
      :style="{ backgroundColor: !showMenu ? '#f4f4f6' : 'rgba(244,244,246,0)' }"
      @close="onClose"
    >
      <div style="display: flex; height: 100vh; overflow: hidden">
        <div>
          <a-card
            title="常用"
            style="height: 100%; width: 200px; position: relative"
          >
            <template #extra>
              <AsyncIcon
                type="text"
                ...
                :class="['setting', { hovered: isHovered }]"
                @mouseover="isHovered = true"
                @mouseleave="isHovered = false"
                size="15px"
                @click="startSettingMenu"
              />
            </template>
            <div
            ...
            >
              <a
                v-if="item.commonUse !== 0 && !item.hideInMenu"
                style="cursor: pointer; width: 100%; height: 35px; line-height: 35px; padding: 0 10px"
                :href="item.path"
                :class="['sub-menu', { hovered: isChildren }, 'block', 'text-black']"
                :key="item.id"
                @mouseover="isChildren = true"
                @mouseleave="isChildren = false"
                @click.prevent.stop="selectMenu(item)"
              >
                <div
                ...
                >
                  ...
                  <div>
                    <AsyncIcon
                      v-if="settingMenu"
                      @click.stop="commonUse(item)"
                      :style="{ color: '#1677FF' }"
                      id="commonUse"
                      icon="PushpinOutlined"
                      size="15px"
                      style="text-align: right; transform: rotate(-40deg)"
                    />
                  </div>
                </div>
              </a>
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <div>
                    <AsyncIcon
                      @click.stop="commonUseFixed(item)"
                      :style="{ color: item.commonUse !== 0 ? '#1677FF' : '#a7a7a8' }"
                      id="commonUse"
                      icon="PushpinOutlined"
                      size="15px"
                      style="text-align: right; transform: rotate(-40deg)"
