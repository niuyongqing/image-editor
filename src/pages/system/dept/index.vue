 ...
 <div>
   <a-card style="margin-top: 10px">
     ...
     <div class="dept-table-container" ref="deptTableContainer">
       <a-table   :data-source="dataSource" :default-expand-all-rows="true" :pagination="false" :columns="columns" row-key="deptId" :loading="tableLoading" bordered :scroll="{ y: tableHeight, x: '100%',virtual: true }" >
         <template #bodyCell="{ column, text, record }" :style="record.style">
           ...
           <div v-if="column.dataIndex === 'option'">
             <a-button type="link" @click.stop="add(record)" color="#2db7f5" v-if="checkPermi(['system:dept:add'])">添加</a-button>
             <a-button type="link" @click.stop="edit(record)" v-if="checkPermi(['system:dept:edit'])">编辑</a-button>
             <a-popconfirm :title="'确定要删除部门吗？'" @confirm="confirmDeleteDept(record)" v-if="checkPermi(['system:dept:remove']) && record.parentId !== 0">
