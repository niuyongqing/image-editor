...
function getMenu() {
  getRoleApi({}).then(res => {
    ...
    const menuList = generateTreeRoutes(res.data).routeData || [];
    menuList.forEach((item) => {
      //  工作台禁用且默认选中
      if (item.path === '/platform/dashboard') {
        item.disabled = true;
...
<template>
  <div>
    <a-form layout="inline" :model="formState" @finish="handleFinish" @finishFailed="handleFinishFailed">
      ...
      <a-form-item>
        <a-select ref="select" v-model:value="formState.status" style="width: 200px;text-align: left" allow-clear
          placeholder="角色状态">
    ...
    <a-card style="margin-top: 10px;padding: 0">
      <div class="table-container" ref="tableContainer" style="text-align: left">
        <a-button type="primary" style="margin-bottom: 10px;" @click="addUser"
          v-has-permi="['system:platform:role:add']">新增</a-button>
        <a-table bordered :dataSource="tableData" rowKey="roleId" :columns="columns" :pagination="false"
          :scroll="{ y: tableHeight, x: '100%', virtual: true }" :loading="tableLoading" :expandedRowKeys="expandedKeys"
          @expandedRowsChange="getRoleUserList">
          <template #expandedRowRender="{ record }">
            <p style="margin: 0">
              <a-table bordered :dataSource="record.dateil ? record.dateil : []" rowKey="userId" :columns="userColumns"
                :loading="userTableLoading" :pagination="false">
          ...
          <template #bodyCell="{ column, record }">
            <div v-if="column.dataIndex === 'status'"><a-switch v-if="record.userName !== 'admin'"
                v-model:checked="record.status" checkedValue="0" unCheckedValue="1" @change="setStatus(record)"
                :disabled="!checkPermi(['system:platform:role:edit'])" /></div>
            <div v-if="column.dataIndex === 'option'">
              <a-button @click="copy(record)" type="text" color="amber"
                v-if="checkPermi(['system:platform:role:edit'])">复制权限</a-button>
              <a-button @click="allocationRole(record)" type="text" color="emerald"
                v-if="checkPermi(['system:platform:role:edit'])">分配用户</a-button>
              <a-button @click="edit(record)" type="text" color="blue"
                v-if="checkPermi(['system:platform:role:edit'])">编辑</a-button>
              <a-popconfirm title="确定删除吗？" ok-text="Yes" cancel-text="No" @confirm="del(record)"
                v-if="checkPermi(['system:platform:role:del'])"><a color="red">删除</a></a-popconfirm>
            </div>
          </template>
        </a-table>
