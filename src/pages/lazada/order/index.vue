            ...
            <a-form :model="formState">
              ...
              ">
                        ...
                        <div style="display: flex">
                            ...
                            <selectComm :options="materialflowType" :fieldObj="materialflowObj"
                                @backSelectAll="materialflowSelectAll" @backSelectItem="materialflowSelectItem">
                            </selectComm>
                        </div>
                    </div>
                </a-form-item>
        ...
        <a-card>
            ...
            <div>
                <a-pagination style="margin: 20px 0 10px 0; text-align: right" :show-total="(total) => `共 ${total} 条`"
                    ...
                    :total="paginations.total" class="pages" :show-quick-jumper="true" @change="queryList"
                ...
                <a-table bordered :columns="columns" :data-source="data" :scroll="{ x: 2000 }" :expand-column-width="0"
                    :defaultExpandAllRows="true" :loading="tableLoading" :rowKey="(record) => record.key"
                ...
                <a-pagination style="margin-top: 10px; text-align: right" :show-total="(total) => `共 ${total} 条`"
                    v-model:current="paginations.pageNum" v-model:pageSize="paginations.pageSize"
                    :total="paginations.total" class="pages" :show-quick-jumper="true" :showSizeChanger="true"
                    @change="queryList" :pageSizeOptions="[50, 100, 200]" />
            </div>
        </a-card>
...
import { asyncOrder, queryShop, queryAreaName, orderList, historyOrderList } from "@/pages/lazada/order/api/order"
import { accountCache } from '@/pages/lazada/order/api';
