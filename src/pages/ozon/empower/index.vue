    ...
    >
      <template #formItemBox>
        <a-form-item label="账号：" name="account">
          <a-input allowClear placeholder="请输入账号" v-model:value="formState.account" />
        </a-form-item>
        <a-form-item label="别名：" name="alias">
          <a-input allowClear placeholder="请输入别名" v-model:value="formState.alias" />
        </a-form-item>
    ...
    >
      <template #leftTool>
        <a-button
          ...
          v-has-permi="[`system:platform:ozon:shop:add`]"
          >新增店铺</a-button
        >
        <a-button
          @click="editnameType = true"
          type="primary"
          v-has-permi="[`system:platform:ozon:shop:update`]"
          >批量修改简称</a-button
        >
        <a-button
          type="primary"
          @click="editWarehouseVisible = true"
          v-has-permi="[`system:platform:ozon:shop:update`]"
          >批量修改仓库</a-button
        >
        <a-button
          type="primary"
          @click="exportInfo"
          v-has-permi="[`system:platform:ozon:shop:export`]"
          >导出</a-button
        >
      </template>
      <template #bodyCell="{ column, record, index }">
        ...
        <template v-if="column.dataIndex === 'pushRatio'">
          <a-input-number
            placeholder="1 - 100"
            :disabled="record.isPushStock == 0"
            class="w-full"
            v-model:value="record.pushRatio"
            :min="1"
            :max="100"
            :precision="0"
        ...
        <template v-if="column.dataIndex === 'alias'">
          <a-input
            ...
            @blur.stop="editSimpleName(record)"
          ></a-input>
        </template>
        <template v-if="column.dataIndex === 'store'">
          <a-select
            ...
            :options="warehouse"
            placeholder="目标仓储类别"
            @change="(e) => editMeansKeepGrain(e, record)"
            :disabled="hasPermi"
          ></a-select>
    ...
    >
      ...
      >
        ...
        <a-form-item label="店铺ID:" name="clientId">
          <a-input
            v-model:value="ruleForm.clientId"
            placeholder="请输入店铺ID"
          ></a-input>
        </a-form-item>
        <a-form-item label="店铺秘钥:" name="apiKey">
          <a-input
            v-model:value="ruleForm.apiKey"
            placeholder="请输入店铺秘钥"
          ></a-input>
          <span style="color: #a9a9a9"
            >请参考格式：cf289f4b-c7fe-4b0f-9880-916848418afb</span
          >
        </a-form-item>
        <a-form-item label="店铺简称:" name="simpleName">
          <a-input
            v-model:value="ruleForm.simpleName"
            placeholder="请输入店铺简称"
          ></a-input>
        </a-form-item>
        <a-form-item label="店铺别名:" name="alias">
          <a-input
            v-model:value="ruleForm.alias"
            placeholder="请输入店铺别名"
          ></a-input>
        </a-form-item>
      </a-form>
    ...
    >
      <a-descriptions :column="1">
        <a-descriptions-item label="示例">
          <a-table
            :data-source="editWarehouseTypeTableData"
            style="width: 100%"
            bordered
            :pagination="false"
        ...
        <a-descriptions-item label="上传">
          <a-upload
            ...
            ref="fileUploadWarehouseType"
            :headers="headers"
            accept=".xlsx, .xlsm, .xls"
            :maxCount="1"
            v-model:file-list="storeFileList"
          >
...
const rules = {
  ...
  apiKey: [{ required: true, message: "请输入店铺秘钥D", trigger: "blur" }],
...
const hasPermi = computed(
  () => checkPermi(["system:platform:ozon:shop:update"]) || checkRole("admin")
);
const getList = () => {
  loading.value = true;
  ...
  list(params).then((res) => {
    tableData.value =
      res?.rows?.map((item) => {
        item.forbidSale = item.forbidSale
          ? item.forbidSale.split(",").map((value) => {
              return Number(value);
            })
          : [];
        return item;
