  ...
  function getAddLogic() {
    if (radio.value === '1') {
      ...
      return function (item) {
        item.oldPrice = absoluteNum.value
      }
    } else {
      // 相对 按数量或百分比增减
      ...
      switch (addType.value) {
        case '1':
          addFn = function (item) {
            item.oldPrice += relativeNum.value
          }
          break
        case '2':
          addFn = function (item) {
        ...
        case '5':
          addFn = function (item) {
            item.oldPrice *= 1 + relativeNum.value / 100
          }
          break
        case '6':
          addFn = function (item) {
      ...
      switch (saveType.value) {
        ...
        case '2':
          saveFn = function (item) {
            item.oldPrice = Math.round(item.oldPrice)
          }
          break
        case '3':
          saveFn = function (item) {
            item.oldPrice = Math.ceil(item.oldPrice)
          }
