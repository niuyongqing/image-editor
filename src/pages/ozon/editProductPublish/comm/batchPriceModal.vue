<template>
    <div id="batchPriceModalCont">
        <a-modal :open="batchPriceVis" :maskClosable="false" @cancel="handleCancel" :width="'30%'" :keyboard="false"
            title="修改售价">
...
<script setup name='batchPriceModal'>
import { ref, reactive, onMounted, computed, watchPostEffect } from 'vue'
import { message } from "ant-design-vue";

const props = defineProps({
    batchPriceVis: Boolean,
    rowPrice: String,
});
const emit = defineEmits(["handleBatchPriceModalClose","backPrice"]);
...
const handleCancel = () => {
    emit("handleBatchPriceModalClose")
...
const onSubmit = () => {
    ...
    console.log('计算结果:', props.rowPrice, calculateResult.value);
    emit("backPrice",form.price)
    handleCancel()
}
...
const calculateResult = computed(() => {
    if (optionForm.radioValue === 2) {
        const oldPrice = parseFloat(props.rowPrice);
