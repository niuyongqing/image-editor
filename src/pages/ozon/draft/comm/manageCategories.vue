<template>
    <div id="typeManage" class="typeManage">
        <a-modal v-model:open="modalOpen" title="分类管理" :width="500" :keyboard="true" :maskClosable="false">
            ...
            <div id="box-tree" class="box-tree">
                <a-tree v-if="treeData.showTree.length > 0" key="classificationTree_typeManage" defaultExpandAll
                    v-model:expanded-keys="treeData.expandedKeys" :tree-data="treeData.showTree"
                    :selectedKeys="treeData.selectedKeys" :fieldNames="{
                        children: 'childList',
                        title: 'name',
                        key: 'id'
                    }" :show-line="true" :show-icon="false" @select="selectNode">
                    <template #title="{ id, name, parentId, level }">
                        <div class="node-template-box" @mouseenter="nodeMouseenter(id)"
                            @mouseleave="nodeMouseleave(id)">
                            <div class="title-content">
                                <a-input v-if="treeData.editNode && id === treeData.editNode.id"
                                    v-model:value="treeData.editNode.name" style="width: 70%;" placeholder=""
                                    ref="inputRef" allowClear @blur="editFn" />
                                <div class="node-name" v-else>{{ name }}</div>
                            </div>
                            <div class="btn-content" v-show="treeData.hoverId === id">
                                <CheckOutlined v-if="treeData.editNode && id === treeData.editNode.id" @click="editFn"
                                    class="typeManage-btn-icon" />
                                <a-space v-else>
                                    <PlusOutlined class="typeManage-btn-icon" @click="addIcon({ id, name, parentId })"
                                        v-if="level < 3" />
                                    <FormOutlined class="typeManage-btn-icon" @click="editIcon({ id, name, parentId })"
                                        v-if="id !== '0'" />
                                    <a-popconfirm v-if="id !== '0'" title="是否删除该节点？" ok-text="是" cancel-text="否"
                                        @confirm="delIcon({ id, name, parentId })">
                                        <DeleteOutlined class="typeManage-btn-icon" />
                                    </a-popconfirm>
                                </a-space>
                            </div>
                        </div>
...
import { addClass, delClass, editClass, getClassList } from '@/components/classificationTree/api.js';
