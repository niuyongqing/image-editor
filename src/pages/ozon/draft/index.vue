<template>
  <div class="flex" id="draftCont">
    <!-- 左侧边栏 -->
    <SideBar
      class="w-70"
      :total-count="totalCount"
      :publish-failed-count="publishFailedCount"
      @draft-emit="handleDraftEmit"
      @wait-emit="handleWaitEmit"
      @online-emit="handleOnlineEmit"
    />
    <div style="width: calc(100% - 250px)">
      <div id="draft" ml-10px>
        <appTableForm
          ref="formRef"
        ...
        >
          ...
          <template #rightTool>
            ...
            <a-button type="primary" @click="navDataCrawli">
              采集数据
            </a-button>
          ...
          {{ index + 1 + (paginations.pageNum - 1) * paginations.pageSize }}
            ...
            <template v-if="column.dataIndex === 'sku'">
              <div class="record-sku-container pb-30px">
                ...
                >
                  <div text-left>
                    <a-button type="link" @click="copyText(item.offerId)">{{
                      item.offerId
                    }}</a-button>
                  </div>
                </div>
            ...
            <template v-if="column.dataIndex === 'option'">
              <a-space direction="vertical">
                <a-button type="link" @click="moveToPending(record)">
                  移入待发布
                </a-button>
                <a-button type="link" @click="editProduct(record)">
                  编辑
                </a-button>
                <a-button type="link" @click="publishProduct(record)">
                  发布
                </a-button>
                <a-dropdown>
                  <a-button type="link">
    ...
    <!-- 批量编辑-提示弹窗 -->
    <BatchEditPrompt ref="batchEditPromptRef" :shopAccount="shopAccount">
    </BatchEditPrompt>
    ...
    <!-- 批量加水印 -->
    <BatchAddWatermarkModal
      v-model:open="watermarkModalOpen"
      @ok="watermarkOk"
    />
...
const publishProduct = (row = {}) => {
  Modal.confirm({
    ...
    onOk: async () => {
      batchPublishToPlatform({
        publishRequestList: [
          { account: row.account, gatherProductId: row.gatherProductId },
        ],
      }).then((res) => {
        message.success("操作成功");
        getList();
    ...
    case 6:
      Modal.confirm({
        ...
        onOk: async () => {
          ...
          const gatherProductIdList = selectedRowList.value.map(
            (item) => item.gatherProductId
          );
          ozonDeleteProduct({
...
function watermarkOk(form) {
  ...
  batchAddWatermarkToDraftProductApi(params)
    .then((res) => {
      selectedRowList.value = [];
      selectedRowKeys.value = [];
      getList();
