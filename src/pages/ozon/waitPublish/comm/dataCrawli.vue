<template>
  <div id="">
    <a-modal :open="editPriceVisible" :maskClosable="false" @cancel="handleCancel" :width="'70%'" :keyboard="false"
      title="数据采集" destroy-on-close>
      <a-card>
        <a-steps :current="1" :percent="30" :items="items" />
      </a-card>
      <a-card title="采集地址--单品采集" class="text-left my-2.5">
        <a-textarea v-model:value="dataUrl" placeholder="请填写产品的网址,多个网址用Enter换行" :auto-size="{ minRows: 4 }" />
        <div class="flex mt-2.5 justify-between">
          ...
          <div>
            <a-button type="primary">开始采集</a-button>
      ...
      <a-card>
        ...
        <a-table :data-source="tableData" style="width: 100%;" :scroll="{y: 400 }" bordered :columns="columns" :pagination="false"
...
import { dataGathe } from "../../config/commDic/defDic"
...
const emit = defineEmits(["handleDataCrawliClose"]);
const items = ref([
  {
    title: '数据采集',
    status: 'wait',
