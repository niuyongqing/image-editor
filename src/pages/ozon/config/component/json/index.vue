...
<template>
  <div class="mobile-detail-editor">
          ...
          <div :class="['content', pointerEventsNone && 'pointer-events-none']">
            <template v-if="moduleList.length">
              ...
              >
                ...
                >
                  <a-textarea
                    v-model:value="item.title.content"
                    placeholder="点击输入标题"
                    :style="item.title.styles"
                    class="titleAreas"
                    :auto-size="{ minRows: 1, maxRows: 3 }"
                  />
                  <a-textarea
                    v-model:value="item.text.content"
                    :style="item.text.cStyles"
                    placeholder="点击输入内容"
                    class="areas"
                    :auto-size="{ minRows: 1 }"
                  />
                ...
                <div
                  v-else-if="item.type === 'text-image'"
                ...
                >
                  <template v-if="item.imgText.dataType != 'chess'">
                    <div :class="getClassName(item.imgText.dataType)">
                      <div
                        class="item"
                        v-for="(imgItem, index) in item.imgText.dataList"
                        :key="index"
                      >
                        <div class="top">
                          <div
                            v-if="!imgItem.src.length"
                            class="tip"
                          >
                            <PictureOutlined :style="{ fontSize: '60px', color: '#a0a3a6' }" />
                        ...
                        <div class="textareas">
                          <a-textarea
                            v-model:value="imgItem.title.content"
                            placeholder="点击输入标题"
                            :style="imgItem.title.styles"
                            class="titleAreas"
                            :auto-size="{ minRows: 1, maxRows: 3 }"
                          />
                          <a-textarea
                            v-model:value="imgItem.text.content"
                            :style="imgItem.text.cStyles"
                            placeholder="点击输入内容"
                            class="areas"
                            :auto-size="{ minRows: 1 }"
                          />
                ...
                <!-- 提示模块 -->
                <div
                  v-else-if="item.type === 'placement-area'"
          ...
          <a-form class="rightForm">
            <template v-if="activeModule.type === 'text'">
              ...
              <a-form-item label="文字大小:">
                <a-select
                  v-model:value="activeModule.title.styles.fontSize"
                  :options="sizeOption"
                ></a-select>
              </a-form-item>
              <a-form-item label="文字颜色:">
                <div class="textColors">
                  <div
                    v-for="(colorItem, i) in colorList"
                    :class="activeModule.title.active == i ? 'cheacd' : ''"
                    :key="i"
                    @click="handleTextColors(colorItem, i, activeModule.title.styles, activeModule.title)"
                    class="colorItems"
                  ></div>
                </div>
              </a-form-item>
              <a-form-item
                label="对齐方式"
                style="border-bottom: 1px dotted #ccc; padding-bottom: 20px"
              >
              ...
              <a-form-item label="文字大小:">
                <a-select
                  v-model:value="activeModule.text.cStyles.fontSize"
                  :options="sizeOption"
                ></a-select>
              </a-form-item>
              <a-form-item label="文字颜色:">
                <div class="textColors">
                  <div
                    v-for="(colorItem, i) in colorList"
                    :class="activeModule.text.active == i ? 'cheacd' : ''"
                    :key="i"
                    @click="handleTextColors(colorItem, i, activeModule.text.cStyles, activeModule.text)"
                    class="colorItems"
                  ></div>
                </div>
              </a-form-item>
              ...
              >
                <div class="flex items-center justify-between">
                  <div class="flex items-center">
                    <div
                      @click="itemImgs.open = !itemImgs.open"
                  ...
                  <div>
                    ...
                    <a-tooltip>
                      ...
                      <DeleteOutlined
                        @click="delImgs(i)"
                        v-if="activeModule.img.length > 1"
                        class="ml-2.5"
                      />
                    </a-tooltip>
                  </div>
                ...
                <div :class="itemImgs.open ? 'showImgCont' : 'noneImgCont'">
                  <a-form-item label="添加图片:">
                    ...
                    <a-upload
                      ...
                      @change="e => uploadChange(e, itemImgs)"
                      :max-count="1"
                    >
                      <div>
                        <PlusOutlined />
                  ...
                  <a-form-item label="跳转链接:">
                    <a-input
                      v-model:value="itemImgs.jumpUrl"
                      placeholder="请填写到ozon.ru的链接"
                    />
                  </a-form-item>
                  ...
                  <a-form-item label="文本描述:">
                    <a-input
                      v-model:value="itemImgs.alt"
                      placeholder="浏览器中图片加载时的文字"
                    />
                  </a-form-item>
                </div>
              </div>
            ...
            <template v-if="activeModule.type === 'text-image'">
              ...
              <a-form-item label="图文类型:">
                <a-select
                  v-model:value="activeModule.imgText.dataType"
                  @change="changeTextImg(activeModule.imgText.dataType)"
                  :options="typeList"
                ></a-select>
              </a-form-item>
              <!-- 图片 -->
              <div
                v-for="(itemImgs, i) in activeModule.imgText.dataList"
                :key="i"
                style="margin-top: 24px"
              >
                <div class="flex items-center justify-between">
                  ...
                  <div>
                    ...
                    <a-tooltip>
                      <template #title>删除组</template>
                      <DeleteOutlined
                        @click="batchDelImgs(i)"
                        v-if="activeModule.imgText.dataList.length > 1"
                      />
                    </a-tooltip>
                  </div>
                </div>
                <div :class="itemImgs.open ? 'showImgCont' : 'noneImgCont'">
                  ...
                  <a-form-item label="图片放置:">
                    <a-radio-group
                      v-model:value="itemImgs.imgTextStyle.width"
                    ...
                    <a-radio-group
                      v-model:value="itemImgs.imgTextStyle.width"
                      @change="changeImgTextStyle($event, itemImgs.imgTextStyle)"
                      v-else
                    >
                ...
                <div :class="itemImgs.open ? 'showImgCont' : 'noneImgCont'">
                  ...
                  <a-form-item label="文字大小:">
                    <a-select
                      v-model:value="itemImgs.title.styles.fontSize"
                      :options="sizeOption"
                    ></a-select>
                  </a-form-item>
                  <a-form-item label="文字颜色:">
                    <div class="textColors">
                      <div
                        v-for="(colorItem, i) in colorList"
                        :class="itemImgs.title.active == i ? 'cheacd' : ''"
                        :key="i"
                        @click="handleTextColors(colorItem, i, itemImgs.title.styles, itemImgs.title)"
                        class="colorItems"
                      ></div>
                    </div>
                  </a-form-item>
                  ...
                  <a-form-item label="文字大小:">
                    <a-select
                      v-model:value="itemImgs.text.cStyles.fontSize"
                      :options="sizeOption"
                    ></a-select>
                  </a-form-item>
                  <a-form-item label="文字颜色:">
                    <div class="textColors">
                      <div
                        v-for="(colorItem, i) in colorList"
                        :class="itemImgs.text.active == i ? 'cheacd' : ''"
                        :key="i"
                        @click="handleTextColors(colorItem, i, itemImgs.text.cStyles, itemImgs.text)"
                        class="colorItems"
                      ></div>
                    </div>
                  </a-form-item>
    ...
    <batchModify
      :showEdit="showEdit"
      @handleBatchModifyClose="showEdit = false"
...
<script setup>
  ...
  defineOptions({ name: 'MobileJSONEditor' })
  ...
  function addImgs() {
    ...
    activeModule.value.img.push(cloneDeep(obj))
  }
  ...
  function batchAddImgs() {
    ...
    activeModule.value.imgText.dataList.push(cloneDeep(obj))
  }
  ...
  const changeTextImg = type => {
    ...
    moduleList.value.forEach(item => {
      if (item.type === 'text-image') {
        ...
        // 批量添加深拷贝对象
        item.imgText.dataList.push(...Array.from({ length: config.count }, () => cloneDeep(baseObj)))
  ...
  function moduleUp(i) {
    // 置换反应
    ;[moduleList.value[i], moduleList.value[i - 1]] = [moduleList.value[i - 1], moduleList.value[i]]
    // 更新当前选中的模块位置
    activeModuleIndex.value = i - 1
    // 更新top位置
    scroll(moduleList.value[i - 1].id)
  }
  function moduleDown(i) {
    ;[moduleList.value[i], moduleList.value[i + 1]] = [moduleList.value[i + 1], moduleList.value[i]]
    activeModuleIndex.value = i + 1
    scroll(moduleList.value[i + 1].id)
  }
        ...
        case 'text-image':
          ...
          if (item.imgText.dataType !== 'chess') {
            obj.blocks = item.imgText.dataList.map(e => {
              ...
              base.img.position = item.imgText.dataType === 'billboard' ? picPosition[e.imgTextStyle.width] : tImgMenu[e.imgTextStyle.width]
  ...
  function openEditor() {
    ...
    content.forEach(item => {
      if (item.widgetName === 'raTextBlock') {
        ...
        let imgObj = cloneDeep(imgDefaulet)
        if (item.type === 'roll') {
          ...
          let imgTextObj = cloneDeep(imgTextDefaulet)
