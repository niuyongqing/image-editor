    ...
    <div style="position: fixed;top: 11%;right: 3%;">
      <a-timeline>
        <a-timeline-item v-for="item in anchorList" :key="item.id" :color="item.turnRed ? 'red' : 'blue'"><a-button
            type="text" :danger="item.turnRed" @click="scroll(item.id)">{{ item.label
...
import { accountCache, categoryAttributes, add, tempSaveOrUpdate, templateList, brandDatabase, relationDetail, dataBaseDetail } from "../config/api/product";
...
const editCategoryModalRef = ref(null) // 分类关联弹窗
...
const feedBackData = (categoryObj) => {
  ...
  dataBaseDetail(params).then(res => {
