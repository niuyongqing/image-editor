  ...
  <div id="OzonNewVariantInfoCont">
    ...
    >
      ...
      >
        <template #extra>
          <div
            style="padding: 3px 0; color: #99999a"
            class="mr-2.5 float-right"
          >
            <a-select
            ...
            <div v-if="tableData.length > 0">
              <a-card class="mb-2.5 ml-2.5">
                <div v-if="imgHeaderList.length > 0">
                  <div
                    v-for="(e, i) in imgHeaderList"
                    :key="i"
                  >
                    <div>
                      <span>{{ e.title }}:</span><span style="margin-left: 10px">{{ item[e.title] }}</span>
                    </div>
                  </div>
                </div>
                <span
                  v-if="item.imageUrl"
                  class="block mt-2.5"
                  >{{ item.imageUrl.length }}/30</span
                >
                <dragUpload
                  @changeImg="list => changeImg(list, item)"
                  @singleSelectImg="e => singleSelectImg(e, item)"
    ...
    for (let i = 0; i < tableData.value.length; i++) {
      ...
      for (let key in tableData.value[i]) {
        if (
          ...
          (item.selectType === 'multSelect' && tableData.value[i][key] === item?.modelValue?.map(val => val.label).join(','))
        ) {
          hasValueFour = true
          break
        }
