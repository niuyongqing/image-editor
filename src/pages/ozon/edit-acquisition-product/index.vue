  ...
  >
    <div class="w-19/20">
      ...
      <ozon-base-info
        ref="ozonBaseInfoRef"
        id="ozonBaseInfo"
        :categoryAttributesLoading="categoryAttributesLoading"
        :shopList="shopList"
        :productDetail="productDetail"
        @sendShortCode="sendShortCode"
        @getAttributes="getAttributes"
      ></ozon-base-info>
      <br />
      <!-- 描述信息 -->
      <ozon-new-image-info
        ref="ozonImageInfoRef"
        id="ozonNewImageInfo"
...
<script setup>
  ...
  import OzonNewImageInfo from './components/OzonNewImageInfo.vue'
  import OzonNewVariantInfo from './components/OzonNewVariantInfo.vue'
  ...
  const onSubmit = async type => {
    ...
    if (image.coverUrl !== '' && image.video.length > 0) {
      ...
      coverUrlBaseObj = createAndUpdateBaseObj(image.coverUrl, 100001, 21841, type)
      ...
      for (let attr of newList) {
        ...
        switch (attr.selectType) {
          ...
          case 'select':
            ...
            break
          case 'multSelect':
            mSlect = getMultiSelectValue(attr, item, base, createValueObj, type)
    ...
    let params = {
      ...
      descriptionCategoryId: base.categoryId.secondCategoryId, // 二级id
      typeId: base.categoryId.threeCategoryId // 三级分id
    }
    console.log('params', params)
    loading.value = true
    ozonProductEdit(params)
      .then(res => {
