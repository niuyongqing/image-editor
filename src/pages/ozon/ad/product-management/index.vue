...
<template>
  <div class="text-left">
    <!-- 搜索区 -->
    <AppTableForm v-model:formData="searchForm" reset-set-menu="adManagement" @on-submit="getList">
      <template #formItemRow>
        <a-form-item label="选择日期" name="dateRange">
          <TiledDateSelect v-model:value="searchForm.dateRange" />
        </a-form-item>
        <a-form-item label="店铺账号" name="account">
          <AppShopSelect v-model:account="searchForm.account" :options="accountList"
            :field-obj="{ label: 'simpleName', value: 'account' }" />
        </a-form-item>
        <a-form-item label="启用状态" name="state">
          <TiledSelect v-model:value="searchForm.state" :options="ENABLE_STATE_OPTIONS" />
        </a-form-item>
        <a-form-item label="模糊查询" name="mult">
          <a-form-item-rest>
            <a-space>
              <a-input v-model:value="searchForm.title" placeholder="活动名称" allow-clear />
              <a-input v-model:value="searchForm.id" placeholder="活动 ID, 多个 ID 间用英文逗号隔开" allow-clear />
              <a-input v-model:value="searchForm.sku" placeholder="sku编码, 多个编码间用英文逗号隔开" allow-clear />
            </a-space>
          </a-form-item-rest>
        </a-form-item>
      </template>
    ...
    <a-card class="mt-2">
      <div class="flex justify-between items-center">
        ...
        <AppTablePagination v-model:current="tableParams.pageNum" v-model:pageSize="tableParams.pageSize" :total="total"
          @change="getList" />
      </div>

      <AppTableBox :table-header="DEFAULT_TABLE_COLUMN" :data-source="tableData" :loading="loading"
        reset-set-menu="adProductManagement" stripe ref="tableRef" row-key="id" :scroll="{ x: 'max-content' }"
        :row-selection="{ selectedRowKeys: selectedRowKeys, onChange: onSelectChange }">
        <template #leftTool>
          <a-space>
            <a-button type="primary" :disabled="selectedRows.length === 0" @click="batchEditBidding">批量调整竞价</a-button>
            <a-button type="primary" @click="addOpen = true">添加推广产品</a-button>
            <a-button :loading="syncLoading" @click="sync">同步推广产品</a-button>
          </a-space>
        </template>
        ...
        <template #bodyCell="{ column, record }">
          <template v-if="column.title === '图片'">
            <a-image :src="record.image || EmptyImg" :width="80" :height="80" :fallback="EmptyImg"
              class="object-contain border border-solid border-gray-200" />
          </template>

          <template v-else-if="column.title === '标题/产品 ID'">
            <div>
              <span :title="record.title">{{ record.title }}</span><a-button type="link" @click="copy(record.title)">
                <CopyOutlined />
              </a-button>
            </div>
            <div>
              <a-button type="link" class="p-0!" @click="goOzon(record.id)">{{ record.id
              }}</a-button><a-button type="link" @click="copy(record.id)">
                <CopyOutlined />
              </a-button>
            </div>
          ...
          <template v-else-if="column.title === '启用状态'">
            <a-switch :checked="record.state" checked-value="CAMPAIGN_STATE_RUNNING"
              un-checked-value="CAMPAIGN_STATE_INACTIVE" @change="toggleState(record)" />
          </template>
          <template v-else-if="column.title === '操作'">
            <a-space>
              <a-button type="link" @click="del(record)">移除</a-button>
            </a-space>
          </template>
        </template>

        <template #pagination>
          <AppTablePagination v-model:current="tableParams.pageNum" v-model:pageSize="tableParams.pageSize"
    ...
    <!-- 添加推广产品弹窗 -->
    <AddModal v-model:open="addOpen" @emitData="refresh" />
  </div>
</template>
