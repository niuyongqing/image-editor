<template>
  <div id="OzonNewVariantInfoCont">
    <a-card
      title="SKU信息"
      class="text-left text-16px mt-20px"
      :loading="categoryAttributesLoading"
    >
      <a-card
        title="变种属性"
        class="text-left mx-50 text-16px"
      >
        <div>
      ...
      >
        ...
        >
          ...
          <template #bodyCell="{ column, record, index }">
            ...
            <template v-if="column.dataIndex === 'sellerSKU'">
              <a-input
                v-model:value="record.sellerSKU"
                style="min-width: 200px"
          ...
          >
            ...
            >
              ...
              <a-dropdown>
                <a-button
                  type="link"
                  style="width: 90px; height: 31px; margin-left: 10px"
                >
                  编辑图片
                  <DownOutlined />
                </a-button>
                <template #overlay>
                  <a-menu>
                    <a-menu-item @click="handleEditImagesSize"> 批量修改图片尺寸 </a-menu-item>
                    <a-menu-item @click="handleImageTranslation"> 图片翻译 </a-menu-item>
                    <a-menu-item @click="clearAllImages"> 清空图片 </a-menu-item>
                  </a-menu>
                </template>
              </a-dropdown>
              <span pl-10px>|</span>
              <a-button
                type="link"
                style="width: 90px; height: 31px; margin-right: 70px"
              ...
              >
                ...
                >
                  <template #default>
                    ...
                    >
                      <p>
                        ...
                        <span class="text-#999"> 第一张图片默认为主图，点击图片拖动，即可调整图片顺序。 </span>
                      </p>
                    </div>
                  </template>
                  <template #variantInfo>
                    <!-- 变种主题信息 -->
                    <div
                      v-for="(nameItem, nameIndex) in skuThemeNames(item)"
                      :key="nameIndex"
                    >
                  ...
                  <template #skuInfo>
                    ...
                    <a-dropdown>
                      ...
                      <template #overlay>
                        <a-menu>
                          <a-menu-item @click="applyAllImage(item)"> 所有变种 </a-menu-item>
                        </a-menu>
                      </template>
                    </a-dropdown>
                  </template>
...
<script setup name="OzonNewVariantInfo">
  ...
  const handleExportAllImages = async () => {
    try {
      const imageList = tableData.value.map(item => item.imageUrl).map(imgItem => imgItem.map(i => i.url.replace(import.meta.env.VITE_APP_BASE_API, '')))
  ...
  watch(
    ...
    val => {
      if (val) {
        ...
        const otherList = aspectList.filter(item => item.id !== 10096 && item.id !== 10097 && item.id !== 4295 && item.id !== 9533).map(item => [item])
        const groupList = [
          // 商品颜色 + 颜色名称
          aspectList.filter(item => item.id === 10096 || item.id === 10097),
          // 俄罗斯尺码和制造商尺码
          aspectList.filter(item => item.id === 4295 || item.id === 9533),
          // 其他数据
          ...otherList
        ]
