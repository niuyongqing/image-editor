  ...
  function getAddLogic() {
    if (radio.value === '1') {
      ...
      switch (addType.value) {
        ...
        case '5':
          addFn = function (item) {
            item.stock *= 1 + relativeNum.value / 100
          }
          break
        case '6':
          addFn = function (item) {
      ...
      switch (saveType.value) {
        case '1':
          saveFn = function (item) {
            item.stock = Number(item.stock.toFixed(2))
          }
          break
        case '2':
          saveFn = function (item) {
            item.stock = Math.round(item.stock)
          }
          break
        case '3':
          saveFn = function (item) {
            item.stock = Math.ceil(item.stock)
          }
