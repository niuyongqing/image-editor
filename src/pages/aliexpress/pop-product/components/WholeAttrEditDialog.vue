<template>
  <!-- 全属性修改弹窗 -->
  <a-modal title="修改属性" :open="dialogVisible" width="45%" destroy-on-close :confirm-loading="submitLoading"
    @cancel="handleClose" @ok="handleConfirm">
    <div class="whole-attr-edit-dialog">
      <div :class="['left', isAttrExpand && 'expand']">
        <div class="left-inner">
          <div class="group" v-for="(group, i) in attrGroupList" :key="i">
            <a-checkbox v-model:checked="group.checkAll" :indeterminate="group.isIndeterminate"
              @change="e => handleCheckAllChange(e, i)">{{ group.title }}</a-checkbox>
            <br />
            <a-checkbox-group v-model:value="group.checkedList" class="ml-2"
              @change="values => handleCheckedChange(values, i)">
              <a-checkbox v-for="item in group.attrList" :value="item.value" :key="item.value">{{ item.label
                }}</a-checkbox>
            </a-checkbox-group>
          </div>
        </div>
      ...
      <div class="right">
        <a-button type="link" :class="['arrow', isAttrExpand ? 'at-right' : 'at-left']"
        ...
        <div class="right-inner">
          <!-- 产品标题 -->
          <a-card v-if="checkedListAll.includes('title')" title="产品标题">
            <a-form :model="titleForm" :label-col="{ style: { width: '72px' } }">
          ...
          <!-- 区域调价 -->
          <a-card v-if="checkedListAll.includes('nationalQuote')" title="区域调价">
            <a-form :label-col="{ style: { width: '72px' } }">
              ...
              <template v-if="nationalQuoteForm.editType === '1'">
                ...
                <a-form-item label="调价方式">
                  <a-select v-model:value="nationalQuoteForm.configurationType" :options="configurationTypeOptions"
                    @change="configurationChange" />
                </a-form-item>
                <a-form-item label="调价区域">
                  <a-checkbox :indeterminate="countriesIsIndeterminate" v-model:checked="countriesCheckAll"
                    @change="handleCountriesCheckAllChange">全选</a-checkbox>
                  <a-checkbox-group v-model:value="checkedCountryList" class="country-checkbox"
                    @change="handleCountriesCheckChange">
                    <a-checkbox v-for="item in countries" :key="item.areaCode" :value="item.areaCode">
                      <span class="inline-block w-30 align-middle truncate">{{ item.areaName }}</span>
                    </a-checkbox>
                  </a-checkbox-group>
                  <a-table :data-source="nationalQuoteTable" :columns="nationalQuoteColumns" bordered
                    :pagination="false" :scroll="{ x: true }">
                    <template #bodyCell="{ record, column }">
                      <a-input-number v-model:value="record[column.dataIndex]" :min="-50" :max="9999" :precision="2"
          ...
          <!-- 运费模版 -->
          <a-card v-if="checkedListAll.includes('freightTemplate')" title="运费模版">
            <a-button type="primary" class="mb-2" @click="syncFreightTemplateList">同步</a-button>
            <p v-if="sellerList.length > 1">共选择 {{ rows.length }} 个产品, 属于不同的店铺, 请分别选择运费模版</p>
            <a-form :model="freightTemplateForm" :label-col="{ style: { width: '180px' } }">
              <a-form-item v-for="seller in sellerList" :key="seller.sellerId" :label="seller.simpleName || '--'"
                :title="seller.simpleName || '--'" class="truncate">
                <a-select v-model:value="freightTemplateForm[seller.sellerId]"
