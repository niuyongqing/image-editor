  ...
  function getAddLogic() {
    if (radio.value === '1') {
      ...
      return function (item) {
        item[props.field] = absoluteNum.value
      }
    } else {
      // 相对 按数量或百分比增减
      ...
      switch (addType.value) {
        case '1':
          addFn = function (item) {
            item[props.field] += relativeNum.value
          }
          break
        case '2':
          addFn = function (item) {
        ...
        case '5':
          addFn = function (item) {
            item[props.field] *= 1 + relativeNum.value / 100
          }
          break
        case '6':
          addFn = function (item) {
      ...
      switch (saveType.value) {
        case '1':
          saveFn = function (item) {
            item[props.field] = Number(item[props.field].toFixed(2))
          }
          break
        case '2':
          saveFn = function (item) {
            item[props.field] = Math.round(item[props.field])
          }
          break
        case '3':
          saveFn = function (item) {
            item[props.field] = Math.ceil(item[props.field])
          }
