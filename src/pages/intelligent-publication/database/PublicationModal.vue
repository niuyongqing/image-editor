  ...
  watch(
    ...
    open => {
      if (open) {
        ...
        form.shops = shopList.map(item => item.account)
      }
    }
  )

  function onCheckAllChange(e) {
    form.shops = e.target.checked ? shopList.map(item => item.account) : []
    indeterminate.value = false
  }

  function checkboxChange(checkedList) {
    const checkedListLength = checkedList.length
    indeterminate.value = checkedListLength > 0 && checkedListLength < shopList.length
    checkAll.value = checkedListLength === shopList.length
  }
