<template>
  <div class="p-16">
    ...
    <div v-if="!wmState.isSelected" class="state-initial">
      <div class="action-section">
        <button class="ie-btn full-width" @click="triggerUpload('add')">
          <svg
            width="16"
            height="16"
            viewBox="0 0 24 24"
    ...
    <div v-else class="state-selected">
      <div class="thumb-row">
        ...
        <button class="replace-text-btn" @click="triggerUpload('replace')">
          替换图片
        </button>
      </div>
...
onMounted(() => {
  if (canvasAPI?.canvas)
    registerWatermarkModule(canvasAPI.canvas, canvasAPI.saveHistory);
});
