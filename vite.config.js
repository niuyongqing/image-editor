    ...
    if (mode === "development" &&
        ...
        proxyObj[env.VITE_APP_BASE_API_DEV] = {
            ...
            rewrite: function (path) {
                return path.replace(new RegExp("^".concat(env.VITE_APP_BASE_API_DEV)), "");
            },
        };
    }
    return {
        ...
        server: {
            port: 80,
            open: true,
            proxy: __assign(__assign({}, proxyObj), (_b = {}, _b[env.VITE_APP_BASE_API] = {
                target: env.VITE_APP_BASE_URL,
                changeOrigin: true,
                rewrite: function (path) {
                    return path.replace(new RegExp("^".concat(env.VITE_APP_BASE_API)), "");
                },
            }, _b)),
        },
        test: {
